...
def create_pie_chart(df: pd.DataFrame, col_name: str, base_path: Path):
    ...
    ax.set_title(
        f'Distribution of {col_name.replace("_", " ").title()}',
        fontsize=14,
        fontweight='bold',
    )
...
def create_bar_chart(
    ...
    ax.set_ylabel('Count')
    ax.set_title(
        f'Top {top_n} {col_name.replace("_", " ").title()}', fontweight='bold'
    )
...
def plot_time_series_overlay(df: pd.DataFrame, col_name: str, base_path: Path):
    ...
    ax.set_ylabel('Number of Bookings')
    ax.set_title(
        f'Yearly Comparison - {col_name.replace("_", " ").title()}',
        fontsize=14,
        fontweight='bold',
    )
    ax.legend()
...
def generate_country_plot(df: pd.DataFrame, base_path: Path):
    ...
    bars = ax.barh(
        ...
        country_data['percentage'],
        color=sns.color_palette('viridis', len(country_data)),
        edgecolor='black',
    )
    ax.set_xlabel('Percentage of Guests (%)')
    ax.set_ylabel('Country')
    ax.set_title(
        'Top 20 Home Countries of Guests', fontsize=14, fontweight='bold'
    )
...
def univariate_eda_plots(
    ...
    for i, col in enumerate(numerical_cols, 1):
        logger.info(
            f"[{i}/{len(numerical_cols)}] Creating histogram for {col}"
        )
        create_histogram_with_box_plot(df, col, base_path)

    logger.info(
        f"Generating {len(categorical_cols)} categorical column plots..."
    )
    ...
    logger.info(
        f"Generating time series plots for {len(datetime_cols)} datetime columns..."
    )
...
def plot_mean_numerical_by_cancellation(
    ...
    dff['is_canceled'] = dff['is_canceled'].map(
    ...
    ax.set_title(
        f'Mean {col_name.replace("_", " ").title()} by Cancellation Status',
        fontsize=14,
        fontweight='bold',
    )
    ax.grid(True, alpha=0.3, axis='y')
...
def plot_mean_categorical_by_cancellation(
    ...
    bars = ax.bar(
        range(len(dff)),
        dff['is_canceled'],
        color=sns.color_palette('RdYlGn_r', len(dff)),
        edgecolor='black',
    )
    ...
    ax.set_title(
        f'Cancellation Rate by {col_name.replace("_", " ").title()}',
        fontsize=14,
        fontweight='bold',
    )
    ax.grid(True, alpha=0.3, axis='y')
...
def correlation_matrix(
    ...
    ax.set_title(
        'Correlation Matrix (Spearman)', fontsize=14, fontweight='bold'
    )
...
def bivariate_eda_plots(
    ...
    logger.info(
        f"Generating {len(numerical_cols)} numerical vs cancellation plots..."
    )
    ...
    categorical_cols_filtered = [
        col for col in categorical_cols if col != 'is_canceled'
    ]
    logger.info(
        f"Generating {len(categorical_cols_filtered)} categorical vs cancellation plots..."
    )
