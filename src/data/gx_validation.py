...
DIST_COLS = [
    "adr", "lead_time", "adults", "children", "babies",
...
ORDERED_DIMENSIONS = [
    ...
    "Uniqueness", "Distribution Profile"
]

ALL_DIMENSIONS = ORDERED_DIMENSIONS + [
    "Outlier Detection", "Relationships Profile"
]
...
def completeness_met(lines, df):
    ...
    lines.append("Overall Completeness:{:.2%}".format(
...
def holiday_met(lines, df):
    ...
    for label, col in [("Days To Next Holiday", "days_to_next_holiday"),
...
def uniqe_met(lines, df):
    ...
    lines.append("Exact Duplicate Pct: {:.2%}".format(
        exact_dupes / len(df) if len(df) > 0 else 0))

    uniqueness_cols = [
        "country", "agent", "adr", "lead_time",
    ...
    for col in uniqueness_cols:
        if col in df.columns:
            ...
            lines.append("  {:33s} :{} distinct ({:.4f} proportion)".format(
                col, nuniq, prop))
...
def corr_met(lines, df):
    corr_cols = [
        "is_canceled", "lead_time", "adr", "adults", "children",
        "stays_in_weekend_nights", "stays_in_week_nights",
        "previous_cancellations", "previous_bookings_not_canceled",
        "booking_changes", "days_in_waiting_list",
        "required_car_parking_spaces", "total_of_special_requests",
        "is_repeated_guest", "is_holiday",
        "days_to_next_holiday", "days_from_last_holiday"
    ]
    ...
    if "is_canceled" in corr_matrix.columns:
        cancel_corr = corr_matrix["is_canceled"].drop("is_canceled").sort_values(
    ...
    holiday_features = ["is_holiday", "days_to_next_holiday", "days_from_last_holiday"]
    target_features = [
        "is_canceled", "lead_time", "adr", "adults",
        "stays_in_weekend_nights", "stays_in_week_nights"
    ]
...
def check_date_consistency(df):
    issues = []
    required = ["arrival_date", "arrival_date_year",
...
def check_reservation_consistency(df):
    ...
    if all(c in df.columns for c in ["adults", "children", "babies"]):
        ...
        if zero_guests > 0:
            issues.append(
                "Bookings with zero total guests (adults+children+babies=0): {} rows".format(zero_guests))
    ...
    if all(c in df.columns for c in ["arrival_date", "reservation_status_date"]):
        ...
        checkout_mask = (df["reservation_status"] == "Check-Out"
                         if "reservation_status" in df.columns
                         else pd.Series(False, index=df.index))
        status_before = (checkout_mask & (res < arr)).sum()
        if status_before > 0:
            issues.append("Check-Out status date before arrival date: {} rows".format(status_before))

    if all(c in df.columns for c in ["is_holiday", "days_to_next_holiday", "days_from_last_holiday"]):
        bad = ((df["is_holiday"] == 1) &
               (df["days_to_next_holiday"] != 0) &
               (df["days_from_last_holiday"] != 0)).sum()
        if bad > 0:
            issues.append(
                "is_holiday=1 but both days_to_next and days_from_last are nonzero: {} rows".format(bad))
...
def _add_statistical_expectations(suite):
    ...
    mean_specs = [
        ("adr", 80, 130), ("lead_time", 80, 130), ("is_canceled", 0.25, 0.45),
        ("is_holiday", 0.0, 0.5),
        ("days_to_next_holiday", 0, 180), ("days_from_last_holiday", 0, 180),
    ]
    ...
    stdev_specs = [
        ("adr", 30, 150), ("lead_time", 50, 150),
        ("days_to_next_holiday", 1, 200), ("days_from_last_holiday", 1, 200),
    ]
...
def _add_null_expectations(suite):
    critical_not_null = [
        ...
        "arrival_date_day_of_month", "arrival_date", "adults", "meal",
        "market_segment", "distribution_channel", "is_repeated_guest",
        "deposit_type", "customer_type", "adr", "reservation_status",
        "reservation_status_date", "is_holiday", "days_to_next_holiday",
...
def _add_non_negative_expectations(suite):
    non_neg_cols = [
        ...
        "total_of_special_requests", "days_to_next_holiday",
        "days_from_last_holiday"
    ]
...
def _add_uniqueness_expectations(suite):
    unique_count_specs = [
        ("country", 50, 250), ("agent", 50, 600),
        ("adr", 1000, 10000),
        ("reserved_room_type", 5, 15), ("assigned_room_type", 5, 15),
    ]
...
EXPECTED_COLUMNS = [
    ...
    "reservation_status_date", "is_holiday",
    "days_to_next_holiday", "days_from_last_holiday",
]
...
def _report_header(lines, results):
    ...
    lines.append("expectations: {} total | {} passed | {} failed".format(
...
def _report_dimension_results(lines, grouped):
    for dim in ORDERED_DIMENSIONS:
        ...
        lines.append("  {} checks | {} passed | {} failed".format(
...
def _report_dimension_summary(lines, grouped, cross_field_issue_count):
    ...
    for dim in ALL_DIMENSIONS:
        ...
        lines.append("  {:25s} : {} ({}/{} passed)".format(
            dim, dim_status, passes, len(dim_results)))

    if cross_field_issue_count == 0:
        lines.append("  {:25s} : PASSED (all cross-field checks clean)".format(
            "Consistency (CrossField)"))
    else:
        lines.append("  {:25s} : FAILED ({} issues found)".format(
