...
def predict_cancellation(
    ...
    # Create arrival_date from components (ensure integers)
    arrival_date = datetime(
        int(arrival_year), int(arrival_month), int(arrival_day)
    )
...
with gr.Blocks(title="Hotel Booking Cancellation Predictor") as demo:
    ...
    gr.Markdown(
        "Predict the likelihood of hotel booking cancellations using advanced "
        "ML"
    )
    ...
    with gr.Row():
        with gr.Column():
            ...
            arrival_month = gr.Slider(
                1, 12, label="Arrival Month", value=7, step=1
            )
            arrival_day = gr.Slider(
                1, 31, label="Arrival Day", value=15, step=1
            )
        ...
        with gr.Column():
            ...
            meal = gr.Dropdown(
                ["BB", "HB", "FB", "SC"], label="Meal Plan", value="BB"
            )
            ...
            is_repeated_guest = gr.Radio(
                ["Yes", "No"], label="Repeated Guest", value="No"
            )
            previous_cancellations = gr.Number(
                label="Previous Cancellations", value=0
            )
            previous_bookings = gr.Number(
                label="Previous Bookings (Not Canceled)", value=0
            )
    ...
    with gr.Row():
        with gr.Column():
            ...
            parking_spaces = gr.Number(
                label="Parking Spaces Required", value=0
            )
        ...
        with gr.Column():
            agent = gr.Textbox(
                label="Agent ID",
                value="Other",
                placeholder="Travel agent ID or 'Other' for direct",
            )
            company = gr.Number(label="Company ID (optional)", value=None)
            is_holiday = gr.Radio(
                ["Yes", "No"], label="Is Holiday Period", value="No"
            )
            ...
            days_from_holiday = gr.Number(
                label="Days from Last Holiday", value=30
            )
