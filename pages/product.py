...
def show():
    ...
    st.markdown(
        "Use this interactive tool to simulate a new booking and predict whether the guest will cancel their stay."
    )
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            lead_time = st.number_input(
                "Lead Time (Days)", min_value=0, max_value=800, value=30
            )
            adults = st.number_input(
                "Adults", min_value=0, max_value=10, value=2
            )
            children = st.number_input(
                "Children", min_value=0, max_value=10, value=0
            )
        with col2:
    ...
    if submit:
        ...
        with col_b:
            if risk_score > 0.50:
                st.error(
                    f"🚨 **HIGH RISK** | This booking has elevated cancellation probability."
                )
                st.markdown(
                    "**Suggested Action:** Trigger an automated follow-up email 7 days before arrival, or consider overbooking this room class."
                )
            else:
                st.success(
                    f"✅ **LOW RISK** | High confidence the guest will arrive successfully."
                )
                st.markdown(
                    "**Suggested Action:** Proceed as normal. Great candidate for upselling spa packages or room upgrades upon arrival."
                )
