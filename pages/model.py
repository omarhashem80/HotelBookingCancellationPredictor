...
def show(metrics_df, eval_data):
    ...
    st.markdown(
        "We continuously test multiple algorithms to ensure the system is highly accurate at flagging at-risk bookings."
    )
    ...
    if metrics_df is not None:
        ...
        if model_comp_path.exists():
            st.image(
                Image.open(model_comp_path),
                caption="F1-Score Comparison Across Models",
            )
    else:
        st.error(
            "No model result data found yet. Please run the training pipeline first."
        )

    st.markdown("### Best Model Detailed Accuracy")
    if eval_data is not None:
        # Extract macro-averaged metrics from classification report
        macro_avg = eval_data.get('classification_report', {}).get(
            'macro avg', {}
        )
        ...
        st.info(
            "To see detailed metrics of the best model, execute the `make train` pipeline."
        )
    ...
    if cm_path.exists():
        st.image(
            Image.open(cm_path),
            caption="Confusion Matrix: Actual vs. Predicted Status",
            width='stretch',
        )
    else:
