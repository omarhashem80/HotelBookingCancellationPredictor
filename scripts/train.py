...
def main() -> None:
	...
	for model_name in selected_models:
		with mlflow.start_run(run_name=model_name):
			...
			business_fnr = false_negative_rate(pd.Series(result.y_true), pd.Series(result.predictions))
			business_cost = cost_sensitive_score(pd.Series(result.y_true), pd.Series(result.predictions))
			metrics = {
				**result.metrics,
